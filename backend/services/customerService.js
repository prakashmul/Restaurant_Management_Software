...
// (fallback for walk-ins with no phone), creating one if none exists yet.
export async function findOrCreateCustomer({ name, phone }, session) {
  const cleanPhone = (phone || '').trim();
  const cleanName = (name || '').trim() || 'Walk-in Customer';
  const validPhone = cleanPhone && cleanPhone !== 'N/A' ? cleanPhone : '';

  const query = validPhone ? { phone: validPhone } : { name: cleanName, phone: '' };
  ...
  const created = await Customer.create([{ name: cleanName, phone: validPhone }], { session });
  return created[0];
}
...
// it only touches orders where customerId is still null.
export async function migrateOrdersToCustomers() {
  try {
    ...
    for (const order of ordersNeedingMigration) {
      const customer = await findOrCreateCustomer({ name: order.customerName, phone: order.customerPhone });
