...
describe('input validation', () => {
  ...
  it('accepts a valid category', async () => {
    const res = await auth(request(app).post('/api/categories')).send({ name: 'Validation Test Category' });
    expect(res.status).toBe(201);
  });

  it('rejects a menu item with a negative price', async () => {
    const res = await auth(request(app).post('/api/menu')).send({
      name: 'Bad Item',
  ...
  it('rejects a menu item missing required fields', async () => {
    const res = await auth(request(app).post('/api/menu')).send({ price: 5 });
    expect(res.status).toBe(400);
  });

  it('coerces numeric strings on inventory creation', async () => {
    const res = await auth(request(app).post('/api/inventory')).send({
  ...
  it('rejects a non-numeric table number', async () => {
    const res = await auth(request(app).post('/api/tables')).send({ number: 'not-a-number' });
    expect(res.status).toBe(400);
  });
