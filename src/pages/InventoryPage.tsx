...
export const InventoryPage = () => {
  ...
  const handleRestock = async (item: InventoryItem) => {
    ...
    const qty = parseFloat(restockAmount[itemId] || '0');
    if (isNaN(qty) || qty === 0) {
      alert('Please enter a valid non-zero amount');
      return;
    }

    const by = performedBy[itemId]?.trim() || 'Anonymous';
    const desc = description[itemId]?.trim() || (qty > 0 ? 'Manual Restock' : 'Manual Deduction');
